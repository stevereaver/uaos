/* cmd_unalias.c — C:unalias — remove a command alias
 *
 * Aliases are stored in the ShellInstance, so the real work is done by the
 * shell built-in.  This native entry exists so that C:unalias resolves
...
void Cmd_UnAlias(NativeCmdCtx *ctx, const char *args)
{
    if (!ctx->dispatch_line || !ctx->shell_extra) {
        PRINT("unalias: shell dispatch not available");
        return;
    }

    char line[CMD_MAX_LINE];
    cmd_scopy(line, "unalias", CMD_MAX_LINE);
    if (args && *args) {
