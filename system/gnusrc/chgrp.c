/* chgrp.c — GNU coreutils 'chgrp' for UAOS gnu: layer
 *
 ...
 * does not actually change group ownership.
 */
...
static int do_chgrp(const char *fname)
{
    ...
    if (opt_verbose) {
        put_s("changed group of '"); put_s(fname);
        put_line("' to root");
    }
    return 0;
}
...
int main(int argc, const char **argv)
{
    ...
    if (nops < 2) { put_line("chgrp: missing operand"); return 1; }

    int rc = 0;
    for (int i = 1; i < nops; i++) {
        const char *fname = uaos_operand(argc, argv, i);
        if (fname) { if (do_chgrp(fname) != 0) rc = 1; }
    }
    return rc;
}
