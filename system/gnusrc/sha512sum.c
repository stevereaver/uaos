/* sha512sum.c — GNU coreutils 'sha512sum' for UAOS gnu: layer
 *
...
static void compute_sha512(int fd, uint32_t sz, int is_stdin, uint8_t out[64])
{
...
static void sha512sum_file(const char *fname)
{
    ...
    if (is_stdin) fd = 0;
        ...
        if (fd < 0) { put_s("sha512sum: "); put_s(fname); put_line(": No such file"); return; }
        struct uaos_stat st; if (uaos_stat(path, &st) == 0) sz = st.size;
    }
    uint8_t digest[64]; compute_sha512((int)fd, sz, is_stdin, digest);
    if (fd > 0) uaos_close((int)fd);
    char hex[129]; hash_to_hex(digest, 64, hex);
...
int main(int argc, const char **argv)
{
    ...
    if (nfiles == 0) { sha512sum_file("-"); return 0; }
    for (int i = 0; i < nfiles; i++) {
        const char *fname = uaos_operand(argc, argv, i);
        if (fname) sha512sum_file(fname);
    }
    return 0;
}
