...
\begin{titlepage}
    ...
    {\normalsize
        ...
        It features transparent M68k JIT emulation and native GNU tool integration.
    }
    \vfill
...
\begin{itemize}
    \item A native microkernel providing Exec, DOS, and Intuition
...
\begin{enumerate}
    \item \textbf{No Linux kernel.}  UAOS boots directly into its own
          x86\_64 microkernel.  No Linux, no POSIX kernel layer.
    ...
    \item \textbf{Kickstart firmware.}  A modular firmware base with
          vectors patched at build time.
\end{enumerate}
...
\texttt{Desktop\_Draw()} renders the full Workbench-style desktop:
\begin{itemize}
    \item \textbf{Menu bar} (top, 20 px) --- Workbench menu labels and clock area.
    \item \textbf{Backdrop} --- solid Amiga grey (\texttt{\#AAAAAA}, R:170 G:170 B:170).
