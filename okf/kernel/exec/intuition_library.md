...
| `OpenWindowTagList` | Implemented | Parses the full `WA_*` tag set, including geometry, title, flags, system gadgets, refresh mode, public/custom screen, min/max bounds, detail/block pens, inner dimensions, screen title, zoom, mouse/repeat queues, backfill, help-group, workbench-window flag, and pointer/busy-pointer state. Stored miscellaneous tags (`WA_Colors`, `WA_Checkmark`, `WA_AmigaKey`, `WA_MenuHelp`, `WA_TabletMessages`, `WA_AutoAdjust`, `WA_NotifyDepth`, `WA_PointerDelay`) are also read. Uses the `NewWindow` pointer as defaults when supplied, including `NewWindow.CheckMark` as the default for `WA_Checkmark`. `WA_AutoAdjust` clamps the initial window position to the containing screen; `WA_MenuHelp` and `WA_TabletMessages` set the `IDCMP_MENUHELP` and `IDCMP_TABLET` flags; `WA_NotifyDepth` enables `IDCMP_NEWSIZE` on z-order changes; `WA_PointerDelay` is applied to `SetWindowPointerA` pointer changes; `WA_Pointer`/`WA_BusyPointer` set the cursor when the window is opened. |
...
| `RefreshWindowFrame` | Implemented | Calls `WM_RepaintWindow` to request a chrome/content repaint. Native sizing-gadget drags and zoom toggles synchronize the guest `Window.LeftEdge`/`TopEdge`/`Width`/`Height` fields before posting `IDCMP_NEWSIZE`. |
...
| Function | Status | Notes |
|----------|--------|-------|
| `HelpControl` | Implemented | Stores the `HC_GADGETHELP` enable/disable state in the window's `IntuitionSlot` and propagates it to every other window that shares the same `WA_HelpGroup`. |
| `StartScreenNotifyTagList` | Implemented | Allocates a screen notification entry, parses the `SN_*` tag list (`SN_Type`, `SN_Flags`, `SN_UserData`, `SN_SignalTask`, `SN_SignalBit`, `SN_Name`), and returns a handle. Matching notifications are dispatched when a screen is opened, closed, or brought to/front/back. |
| `EndScreenNotify` | Implemented | Frees the screen-notification handle returned by `StartScreenNotifyTagList` and returns `TRUE`. |
...
| `GetScreenAttrsA` | Implemented | Reads back the corresponding live screen fields, flags, and stored tag values. `SA_PubName` returns a guest-allocated pointer to the screen's public name string; the pointer is valid for the lifetime of the screen. |
...
`WA_*` and `SA_*` tag values, `WFLG_*` window flags, and screen type flags now match the AmigaOS 3.x definitions. Tags that only need to be readable (e.g., `WA_BackFill`, `WA_Colors`, `SA_BackFill`, `SA_Draggable`, `SA_Exclusive`, `SA_SharePens`, `SA_LikeWorkbench`, `SA_MinimizeISG`) are stored in the internal `IntuitionSlot` / `ScreenSlot` so they can be read back with `GetWindowAttrsA` / `GetScreenAttrsA`. Previously storage-only tags that now have live behaviour include `WA_PointerDelay` (delayed pointer update), `WA_AutoAdjust` (window clamped to screen), `WA_NotifyDepth` (`IDCMP_NEWSIZE` on depth changes), `WA_MenuHelp` / `WA_TabletMessages` (`IDCMP_MENUHELP` / `IDCMP_TABLET` flags), `SA_ErrorCode` (failure-code written on `OpenScreen` failure), `SA_FullPalette` / `SA_ColorMapEntries` (palette entry count), and `SA_Interleaved` (`BMF_INTERLEAVED` bit in the custom `BitMap`). `WA_Zoom` is now enforced: the existing WM zoom gadget toggles the window between the two rectangles in the zoom array. `WA_HelpGroup` / `WA_HelpGroupWindow` are enforced: the F1 key posts an `IDCMP_HELP` message to the focused window or to the designated help-group window. `SA_Parent` is enforced when opening a screen by inheriting the parent screen's dimensions and default pens if the child did not supply them. `SA_DClip` and `SA_Overscan` now define an active constraint rectangle for each screen: `SA_DClip` uses the supplied `Rectangle` directly, while `SA_Overscan` maps to a centred rectangle (`OSCAN_TEXT` 90%, `OSCAN_STANDARD` 95%, `OSCAN_MAX` 98%, `OSCAN_VIDEO` 105%) of the framebuffer. When a screen is opened, `MoveScreen()` is called, or `ScreenPosition()` is invoked, the screen's `LeftEdge`/`TopEdge`/`Width`/`Height` are clamped to remain within this rectangle. `SA_PubSig` and `SA_PubTask` are already enforced as described above, and `SA_PubName` is exposed as a guest-allocated string via `GetScreenAttrsA()`. `WA_Pointer` and `WA_BusyPointer` are now wired during both `OpenWindowTagList()` and `SetWindowAttrsA()`, and `WA_PointerDelay` is honoured in both paths. `StartScreenNotifyTagList()` / `EndScreenNotify()` and `HelpControl()` group propagation are now implemented. `IDCMP_INTUITICKS` is posted at 10 Hz to all windows that request it.

**Backing semantics:** every Intuition screen is backed by a real planar `BitMap` (1–8 bitplanes). `OpenScreen`/`OpenWorkbench` allocate one via `graphics.library` `AllocBitMap` when the guest does not supply `SA_BitMap`, build a `ColorMap` seeded from the default palette plus `SA_Colors`/`SA_Colors32` (the Workbench palette forces pen 0 to grey), and attach a minimal guest `ViewPort` whose `ColorMap` points at it so `ViewPortAddress()` and window colormap lookups resolve it. `Screen.RastPort` and every non-SuperBitMap window `RastPort` point at the screen `BitMap`; window-relative drawing is translated to screen-bitmap coordinates by `blit_surface_from_rastport()` in `graphics_lib.c`.

`UAOS_Intuition_RenderScreenBackdrop()` renders the front screen's `BitMap` through `screen_colormap()` into the framebuffer (running `SA_BackFill` first so pen/hook fills land in the `BitMap`). `UAOS_Intuition_FlushScreenBitmap()` re-renders just the dirty rectangle after each graphics dispatch that wrote planar pixels. The WM draw callback (`intu_draw_fn`) repaints each window's client region from the screen `BitMap` over the host chrome fill. `WA_SuperBitMap` windows keep their own backing `BitMap`, rendered into the client area as before. A `WM_SetVacateFn` hook (`intu_screen_vacate`) erases a window's old rectangle in the screen `BitMap` (pen 0) whenever the window moves, resizes, zooms, or closes, so vacated areas revert to the backdrop instead of showing stale pixels.

`WA_BackFill` / `SA_BackFill` hooks are stored as pointers; if the stored value is a real guest `Hook` pointer it is dispatched via `UAOS_InvokeM68kHook()` using the AmigaOS Hook convention (A0=Hook, A2=RastPort, A1=Rectangle). A small hook value (less than 256) is treated as a pen index and fills via the screen `BitMap` (falling back to a direct framebuffer fill when no `BitMap` exists).
...
| `AllocScreenBuffer` | Implemented | Allocates a `ScreenBuffer` with a `BitMap` (copy or screen bitmap) and a `DBufInfo`.  `SB_COPY_BITMAP` allocates a new BitMap; `SB_SCREEN_BITMAP` reuses the screen's existing BitMap. |
| `FreeScreenBuffer` | Implemented | Frees the `ScreenBuffer`, its `DBufInfo`, and any allocated `BitMap`. |
| `ChangeScreenBuffer` | Implemented | Swaps the screen's BitMap to the `ScreenBuffer`'s BitMap and triggers a WM redraw.  Performs an immediate synchronous swap (UAOS renders synchronously rather than deferring to VBlank). |

Screens are mapped to the single UAOS desktop. Multiple screens are tracked in a small internal table; the frontmost screen with `SHOWTITLE` set controls the title displayed in the native desktop menu bar. `SA_DClip` and `SA_Overscan` define a constraint rectangle that controls the screen's initial geometry and limits subsequent moves via `MoveScreen()` and `ScreenPosition()`. `SA_Colors`, `SA_Colors32`, and `SA_Pens` are parsed into a host RGB palette and applied to the desktop and window chrome. If a screen is opened with `SA_BitMap`, or `SA_BitMap` is later changed via `SetScreenAttrsA()`, the custom planar `BitMap` is rendered into the desktop backdrop through the screen's `ViewPort` `ColorMap`. `StartScreenNotifyTagList()`/`EndScreenNotify()` provide screen-change notifications: notifications are dispatched when screens are opened, closed, or reordered (front/back), and optionally filtered by public name. The screen's public name (`SA_PubName`) is copied into a guest-allocated string that `GetScreenAttrsA()` can return.
...
The native cursor subsystem gained custom-sprite support: a per-window custom pointer can replace the default arrow, and a busy pointer can be activated when the active window requests it. `WA_Pointer` objects are rendered by heuristically detecting a SetPointer-style sprite buffer, a `struct BitMap`, or a `pointerclass` BOOPSI object whose first valid pointer is a BitMap. `SetWindowAttrsA()` now also honours `WA_Pointer` and `WA_BusyPointer`, applying the cursor immediately or respecting `WA_PointerDelay` when both are supplied. `SetPointer()` now correctly skips the Amiga sprite reserved words and passes the caller's x/y hotspot offsets to the cursor renderer.
