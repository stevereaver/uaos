...
- **VirtIO-Net (`kernel/drivers/virtio_net.c`)**: VirtIO network driver supporting the legacy/transitional device (1af4:1000, BAR0 I/O ports) and the modern virtio-1.0 device (1af4:1041, vendor-capability transport with PCI_CFG config-space window fallback for BARs above the 4GB identity map).
